...
import { z } from "zod";
import { syncDependentCourseAccess,TOOLS_SLUG } from "@/lib/access";
...
export async function POST(req:Request){try{
  ...
  if(course.slug===TOOLS_SLUG){if(!(await syncDependentCourseAccess(user.id)))fail(403,"Bạn cần được mở khóa K2 trước khi học khóa sử dụng công cụ");return NextResponse.json({ok:true,dependentAccess:true},{status:201})}
