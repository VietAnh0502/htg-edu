...
export function CourseDetailNav({isK2}:{isK2:boolean}) {
  return <nav className="detail-tabs" aria-label="Điều hướng nội dung khóa học">
    {sections.filter(([label])=>isK2||label!=="Thông tin K2").map(([label,target])=><button type="button" key={target} onClick={()=>document.querySelector(target)?.scrollIntoView({behavior:"smooth",block:"start"})}>{label}</button>)}
  </nav>;
}
